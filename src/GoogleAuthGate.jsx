...
export default function GoogleAuthGate({ children }) {
  ...
  const submitRegister = async (event) => {
    ...
    if (!termsAccepted) {
      setError('Mahar POS အသုံးပြုမှု သဘောတူညီချက်ကို လက်ခံပေးပါ။');
      return;
    }
      ...
      <section className="ms-auth-shell">
        <aside className="ms-auth-brand-panel">
          ...
          <div className="ms-auth-brand-copy">
            <p className="ms-auth-pill">Lifetime Free Plan ပါဝင်သည်</p>
            <h1>
              Mobile Shop ကို
            ...
            <div className="ms-auth-form-wrap">
              <div className="ms-auth-title-block">
                ...
                <p>
                  {mode === 'login'
                    ...
                    : 'ဆိုင်အကောင့်အသစ်ဖွင့်ပြီး Lifetime Free Plan ကို စတင်နိုင်ပါသည်။'}
                </p>
              </div>
              ...
              {registerSuccess ? (
                <div className="auth-register-success ms-auth-success">
                  ...
                  <small>Lifetime Free Plan စတင်ပြီးပါပြီ။ Tenant ID ကို Login form ထဲ auto-fill လုပ်ထားပြီးပါပြီ။</small>
                </div>
              ) : null}
              ...
              {mode === 'login' ? (
                ...
                <>
                  <div className="ms-auth-google-wrap ms-auth-register-google">
                    <div className="auth-google-button ms-auth-google-render" ref={buttonRef} />
                    <p className="ms-auth-google-note">
                      Gmail အသစ်ဖြင့်ဝင်ပါက tenant/shop အသစ်ကို Lifetime Free Plan ဖြင့် auto-create လုပ်ပေးပါမယ်။
                    </p>
                  </div>
                  ...
                  <form className="auth-gate-form auth-register-form ms-auth-form ms-auth-register-form" onSubmit={submitRegister}>
                    ...
                    <div className="ms-auth-trial-note">
                      အကောင့်ဖွင့်ပြီးပါက Tenant ID ထုတ်ပေးပြီး Lifetime Free Plan ကို စတင်အသုံးပြုနိုင်ပါသည်။
                    </div>
                    <label className="ms-auth-terms">
                      <input type="checkbox" checked={termsAccepted} onChange={(event) => setTermsAccepted(event.target.checked)} />
                      <span>Mahar POS အသုံးပြုမှု သဘောတူညီချက်နှင့် ကိုယ်ရေးအချက်အလက်ဆိုင်ရာ မူဝါဒကို လက်ခံပါသည်။</span>
                    </label>
                    <button type="submit" disabled={busy}>
                      {busy ? <Loader2 className="auth-gate-spin" size={19} /> : <UserPlus size={19} />}
                      {busy ? 'Creating tenant…' : 'အခမဲ့အကောင့် စတင်မည်'}
                    </button>
                  </form>
