      ...
      <main className="ps-content">
        ...
        {data && section === 'preferences' ? <section className="ps-panel">
          ...
          <div className="ps-form ps-grid-2">
            ...
            {isMiniMart ? <Toggle label="Money Service Menu ပြမည်" hint="Mini Mart မှာ default မပြပါ။ Cash In / Cash Out သုံးချင်မှ ဖွင့်ပါ။" checked={showMoneyService} onChange={(value) => { try { window.localStorage.setItem('miniMartShowMoneyService', value ? 'true' : 'false'); } catch {} updateForm('preferences', { showMoneyService: value, openingPage: value ? forms.preferences.openingPage : (forms.preferences.openingPage === 'Money Service' ? 'Sale POS' : forms.preferences.openingPage) }); }} disabled={!canManage}/> : null}
