...
async function exportBillers(shopId, since, take) {
  return prisma.$queryRawUnsafe(
    `SELECT id::text AS id,
            ...
            sale_adjust_mode AS "saleAdjustMode",
            sale_adjust_percent AS "saleAdjustPercent",
...
async function exportBillerTransactions(shopId, since, take) {
  return prisma.$queryRawUnsafe(
    `SELECT t.id::text AS id,
            t.transaction_date AS "transactionDate",
            t.created_at AS "createdAt",
            b.name AS "billerName",
            b.type::text AS "billerType",
            t.transaction_type::text AS "transactionType",
            t.amount,
            t.balance_effect_amount AS "balanceEffectAmount",
            t.balance_adjust_mode AS "balanceAdjustMode",
            t.balance_adjust_percent AS "balanceAdjustPercent",
            t.cost_amount AS "costAmount",
            t.profit_amount AS "profitAmount",
            t.payment_method AS "paymentMethod",
            a.name AS "paymentAccountName",
            t.payment_status::text AS "paymentStatus",
            ...
            t.note
       FROM biller_transactions t
       JOIN billers b ON b.id=t.biller_id AND b.shop_id=t.shop_id
       LEFT JOIN money_accounts a ON a.id=t.payment_account_id
       LEFT JOIN users u ON u.id=t.staff_id
      WHERE t.shop_id=$1::uuid
        AND t.updated_at >= $2
...
async function exportDailyReport(shopId, since, take) {
  return prisma.$queryRawUnsafe(
    `WITH biller_daily AS (
       SELECT transaction_date::date AS day,
              COALESCE(SUM(amount) FILTER (WHERE transaction_type='SOLD'),0) AS bill_sold,
              COALESCE(SUM(amount) FILTER (WHERE transaction_type='REFILL'),0) AS bill_refill,
              COALESCE(SUM(profit_amount) FILTER (WHERE transaction_type='SOLD'),0) AS bill_profit
         FROM biller_transactions
        WHERE shop_id=$1::uuid
        GROUP BY 1
     )
     SELECT dc.id::text AS id,
            dc.closing_date AS "businessDate",
            dc.sales_total AS "salesTotal",
            dc.product_profit_total AS "productProfitTotal",
            dc.service_income_total AS "serviceIncomeTotal",
            dc.money_profit_total AS "moneyProfitTotal",
            dc.repair_income_total AS "repairIncomeTotal",
            dc.repair_profit_total AS "repairProfitTotal",
            COALESCE(b.bill_sold,0) AS "billEloadSold",
            COALESCE(b.bill_refill,0) AS "billEloadRefill",
            COALESCE(b.bill_profit,0) AS "billEloadProfit",
            dc.other_income_total AS "otherIncomeTotal",
            dc.expense_total AS "expenseTotal",
            dc.total_profit AS "totalProfit",
            dc.cash_balance AS "cashBalance",
            dc.kpay_balance AS "kpayBalance",
            dc.wave_pay_balance AS "wavePayBalance",
            dc.note,
            dc.closed_at AS "closedAt",
            u.name AS "closedBy"
       FROM daily_closings dc
       LEFT JOIN biller_daily b ON b.day=dc.closing_date
       LEFT JOIN users u ON u.id=dc.closed_by_id
      WHERE dc.shop_id=$1::uuid
        AND dc.closed_at >= $2
      ORDER BY dc.closed_at ASC
      LIMIT $3`,
    shopId,
    since,
    take,
...
async function exportDataset(shopId, dataset, since, limit) {
  ...
  if (dataset === 'money-service') return exportBillerTransactions(shopId, since, take);
  if (dataset === 'billers') return exportBillers(shopId, since, take);
  if (dataset === 'biller-transactions') return exportBillerTransactions(shopId, since, take);
  if (dataset === 'daily-report') return exportDailyReport(shopId, since, take);
